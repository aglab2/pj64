		...
		SetDlgItemText(hDlg,IDC_PLUGIN,GS(NOTE_PLUGIN));
		{
...

			{
				...
				for (p = String; strlen(p) > 0; p += strlen(p) + 1) {
					...
					*(strrchr(Status,'=')) = 0;
					len = strlen(Status);
					if (len > 4 && strnicmp(&Status[len-4],".Sel",4) == 0) { continue; }
					if (len > 8 && strnicmp(&Status[len-8],".Seltext",8) == 0) { continue; }
					if (len > 15 && strnicmp(&Status[len-15],".AutoFullScreen",15) == 0) { continue; }
